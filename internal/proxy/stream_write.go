...
func streamTransformedOpenAIResponses(gc *gin.Context, resp *http.Response, usageTail *tailBuffer, dump *streamDumpState) (int64, error) {
	...
	if rec == nil || rec.MaxBytes() <= 0 {
		cw := &countingWriter{w: io.MultiWriter(gc.Writer, usageTail)}
		err := dslconfig.TransformOpenAIResponsesSSEToChatCompletionsSSE(src, cw)
		return cw.n, err
	}
	...
	cw := &countingWriter{w: io.MultiWriter(gc.Writer, prDump, usageTail)}
	err := dslconfig.TransformOpenAIResponsesSSEToChatCompletionsSSE(src, cw)
	if dump != nil {
...
func streamPassthrough(gc *gin.Context, resp *http.Response, usageTail *tailBuffer, dump *streamDumpState) (int64, error) {
	gc.Status(resp.StatusCode)

	rec := trafficdump.FromContext(gc)
	if rec == nil || rec.MaxBytes() <= 0 {
		tee := io.TeeReader(resp.Body, usageTail)
		n, err := io.Copy(gc.Writer, tee)
		return n, err
	}
